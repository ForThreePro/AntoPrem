...
const saveAudio = async (m, type) => {
  ...
  if (!/audio/.test(mime)) return m.reply(`╭─💖 *『 𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗙𝗘𝗠 』* 💖─╮\n│ ⚠️ *ESO NO ES UN AUDIO*\n╰─────────────────💖`);
...
let handler = async (m, { conn, text, command, isAdmin }) => {
  if (!isAdmin) return m.reply(`╭─💖 *『 𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗙𝗘𝗠 』* 💖─╮\n│ 😡 *SOLO REINAS ADMINS*\n╰─────────────────💖`);

  let chat = global.db.data.chats[m.chat] || {}
  
  switch(command) {
    case 'setwelcome':
      if (!text) return m.reply(`╭─💖 *『 𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗙𝗘𝗠 』* 💖─╮
│ ⚠️ *FALTA TEXTO*
│
│ *Ejemplo:* .setwelcome @name brilla en @group ✨
│ *Variables:* @user @name @group @desc %users @action @date
╰─────────────────💖`);
      chat.welcomeText = text;
      global.db.data.chats[m.chat] = chat
      await global.db.write()
      return m.reply(`╭─💖 *『 𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗙𝗘𝗠 』* 💖─╮
│ ✨ *BIENVENIDA GUARDADA*
╰─────────────────💖
...
> *"Otra reina llegó al club"*`);

    case 'setbye':
      if (!text) return m.reply(`╭─💖 *『 𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗙𝗘𝗠 』* 💖─╮
│ ⚠️ *FALTA TEXTO*
│
│ *Ejemplo:* .setbye @name dejó el club 💅
...
> *"Una reina se fue, pero el brillo sigue"*`);
...
│
│ *Volvio al mensaje FEM por defecto*
╰─────────────────💖`);
...
╰─────────────────💖`);
    ...
    case 'audiowelcome':
      if (!m.quoted) return m.reply(`╭─💖 *『 𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗙𝗘𝗠 』* 💖─╮\n│ ⚠️ *RESPONDE A UN AUDIO*\n╰─────────────────💖`);
